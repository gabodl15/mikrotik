        ...
        <div class="page-container">
            ...
            <div class="page-content">
                ...
                <div class="page-title">
                    <h2><span class="fa fa-arrow-circle-o-left"></span> Listado Usuarios Colas Simples</h2>
                </div>
                <!-- END PAGE TITLE -->
                                         ...
                                         <?php
                                            ...
                                            if(count($ARRAY)>0){   // si hay mas de 1 queue.
                                                for($x=0;$x<count($ARRAY);$x++){
                                                    $limit = $ARRAY[$x]['max-limit'];
                                                    $target = $ARRAY[$x]['target'];
                                                    list($upload,$download) = explode("/",$limit);
                                                    $uploadMegas = round($upload/1024);
                                                    $downloadMegas = round($download/1024);
                                                    $prioridad = $ARRAY[$x]['priority'];
                                                    if ($uploadMegas>="1000") {
                                                        $uploadMegas = "".($uploadMegas/1000)."M";
                                                    }else{
                                                        $uploadMegas = "".$uploadMegas."K";
                                                    }
                                                    if ($downloadMegas>="1000") {
                                                        $downloadMegas = "".($downloadMegas/1000)."M";
                                                    }else{
                                                        $downloadMegas = "".$downloadMegas."K";
                                                    }
                                                    if ($prioridad == "1/1" || $prioridad == "2/2") {
                                                        $segmento = "Dedicado";
                                                    }elseif ($prioridad == "3/3" || $prioridad == "4/4") {
                                                    ...
                                                    $datos_user_queue_simple = '<tr>';
                                                    $datos_user_queue_simple.= '<td>'.$name.'</td>';
                                                    $datos_user_queue_simple.= '<td>'.$target.'</td>';
                                                    $datos_user_queue_simple.= '<td>'.$downloadMegas."/".$uploadMegas.'</td>';
                                                    $datos_user_queue_simple.= '<td>'.$segmento.'</td>';
                                                    $datos_user_queue_simple.= '</tr>';
                                                    echo $datos_user_queue_simple;
                                                }
